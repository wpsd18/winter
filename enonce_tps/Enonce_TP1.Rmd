...
#### Exercice 3 : Distance de Mahanalobis (45 minutes)

Soit $C$ une matrice symétrique dont les valeurs propres sont positives. Soit ${\bf m}$ un vecteur arbitraire dans le plan $\mathbb{R}^2$. On définit la distance de Mahanalobis entre un vecteur ${\bf x}$ et le vecteur ${\bf m}$ de la manière suivante
...
##### Valeur moyenne de la distance de Mahanalobis au carré. 
...
On admet que la valeur moyenne de la distance de Mahanalobis au carré entre ${\bf x_0}$ et l'origine est égale à
...
* Calculer la valeur moyenne du carré de la distance de Mahanalobis entre chaque vecteur simulé dans l'objet `x0` et de centre de `x` (pour la matrice $C$). Compléter le code suyivnt
...
* Pour tout vecteur contenu dans l'objet `x`, calculer le carré de la distance de Mahanalobis séparant ce vecteur au centre de la classe à laquelle il appartient. Compléter le code suivant.
