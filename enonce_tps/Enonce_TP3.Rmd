...
L'objectif de cette séance de travaux pratiques est de comparer les performances de trois méthodes de classification supervisée : les méthodes des $k$-plus proches voisins, de l'analyse discriminante linéaire et un réseaux de neurones à une couche cachée. Il s'agit d'aborder le phénomène de sur-entrainement (overfitting) et de choisir les paramètres des classifieurs permettant d'obtenir les meilleures perfromances sur l'ensemble test.
...
for (k in 1:30){
  ...
  log_loss[k] <- - mean(boo * log(prob_class) - (!boo) * log(1 - prob_class) )
}
