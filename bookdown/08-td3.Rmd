...
$$
En d'autres termes, la perte est égale à 1 si $c({\bf x}) \neq y$, 0 sinon.
...
On suppose que l'échantillon est de taille arbitrairement grande ($n \to \infty$). Sous quelles conditions portant sur $k$ et la taille du voisinage $V({\bf x})$ pourrait-on justifier l'approximation suivante 
...
$$
Pour les cas extrêmes $k = 1$ et $k = n$, donner une approximation de l'espérance conditionnelle de $\hat q({\bf x};k)$ sachant ${\bf x}$. 

Quelle est prédicteur vous paraît le plus exact ? (On suppose que ${\bf x}$ est indépendant de l'échantillon d'apprentissage).

### Question 4

Pour les cas extrêmes $k = 1$ et $k = n$, calculer une approximation de la variance de $\hat q({\bf x};k)$ dans chacun des deux cas.

Quelle est prédicteur vous paraît le moins variable ? Comment peut on choisir $k$ ?


### Question 5. Application au modèle de Hastie et Tibshirani.

Le modèle HT (de Hastie et Tibshirani) consiste à simuler un feu d'artifice gaussien. La première classe est centrée en $(1,0)$ et $10$ sous-classes sont générées selon la loi $N((0,1), {\bf I})$. Les observations sont alors simulées en choisissant une sous-classe $m_k$ au hasard parmi les 10 sous-classes puis en générant des données selon une loi de variance (*sigma2*), $N(m_k, \sigma^2{\bf I})$. La seconde classe obéit au même modèle génératif, mais à partir du point central $(0,1)$.
