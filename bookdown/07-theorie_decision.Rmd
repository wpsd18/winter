...
Sous les mêmes conditions que dans la section précédente, supposons que $y$ est une variable discrète prenant $K$ valeurs. La *formule de Bayes* permet de calculer la probabilité conditionnelle $p(y = k|{\bf x})$  à partir de la loi conditionnelle du vecteur ${\bf x}$ sachant la valeur  $y = \ell$. La formule s'écrit de la manière suivante 

$$
p(y = k|{\bf x}) = \frac{p({\bf x} | y = k ) p(y = k)}{p({\bf x})} = \frac{p({\bf x} | y = k ) p(y = k)}{\sum_{\ell = 1}^K p({\bf x} | y = \ell ) p(y = \ell) } \, , \quad \forall k  = 1, \dots, K.
$$
Lorsque les valeurs de $y$ sont équiprobables, nous avons

$$
p(y = k) = \frac1K \, , \quad \forall k  = 1, \dots, K,
$$
et la formule se réduit à

$$
p(y = k|{\bf x})   = \frac{p({\bf x} | y = k )}{\sum_{\ell = 1}^K p({\bf x} | y = \ell )} \, , \quad \forall k  = 1, \dots, K.
$$
...
$$
 { \bf x }  \mapsto q({ \bf x }) = (q_1({ \bf x }), \dots, q_K({ \bf x }))   \in [0,1]^K \, , \quad {\rm t.q.} \quad \sum_{k = 1}^K q_k({ \bf x }) = 1 \, .
$$
...
$$
Nous verrons ainsi que minimiser la perte moyenne log-loss revient à **minimiser l'information perdue lorsque l'on approche la loi cible $p(y|{\bf x})$ par une machine probabiliste $q({\bf x})$**. 


Nous allons démontrer ce résultat très important pour le cours. Dans un premier temps, montrons un résultat similaire pour une classification binaire, $y \in \{0,1\}$, où $q({\bf x})$ est identifié à la classe $q_1({\bf x})$. Lorsque la variable $y$ est binaire, la fonction de perte moyenne s'écrit de la manière suivante

$$
\mathbb{E}[L(q({\bf x}), y)] = \mathbb{E}[ - y \log q({\bf x}) - (1-y)\log (1 - q({\bf x})) ] \, ,
$$

Pour minimiser la fonction de perte moyenne définie ci-dessus, il suffit de minimiser le terme $\mathbb{E}[L(q({\bf x}), y) | {\bf x}]$, intervenant à l'intérieur de l'intégrale suivante
...
$$
\mathbb{E}[L(q({\bf x}), y )|{\bf x}] =  - (1 - p(y=1|{\bf x})) \log(1 - q({\bf x})) -  p(y=1|{\bf x}) \log(q({\bf x})) \, .
$$

Pour tout $p \in (0,1)$,la fonction $q \in (0,1) \mapsto  - (1 - p) \log(1 - q) -  p \log(q)$ est minimale si et seulement si $q = p$ (par dérivation). Ceci démontre que la solution optimale est  

$$
q({\bf x}) = p(y = 1 | {\bf x}) \, .
$$

Démontrer le résultat pour $K$ classes s'appuie sur un principe similaire. Minimiser la perte moyenne revient à rechercher une machine probabiliste $q({\bf x})$ minimisant l'espérance conditionnelle suivante
...
$$
h( p_1 , p_2 ) =  - \sum_{k = 1}^K p(y = k|{\bf x}) \log q_k({\bf x}) .
$$
...
Pour un modèle de prédiction paramètrique, la perte moyenne log loss peut être directement reliée à une autre grandeur, dont l'importance sera très grande dans le cours de statistique. Il s'agit de la fonction **log-vraisemblance**. 

Considérons un modèle paramètrique $q({\bf x}, \theta)$. Considérons $n$ d'observations $({\bf x}_i, y_i)$, $i = 1, \dots, n$, indépendantes et toutes générées selon le modèle. L'ensemble d'observations est appelé un **échantillon statistique**. Connaissant $\theta$, la loi générative d'une seule observation $({\bf x}, y)$ est décrite par la formule suivante

$$
p({\bf x}, y | \theta) = p({\bf x} | y, \theta) p(y) = \left\{ 
\begin{array}{l}
q({\bf x}, \theta) p(y = 1) & {\rm si } ~ y = 1 \, , \\ 
(1 - q({\bf x}, \theta)) p(y = 0) & {\rm si } ~ y = 0 \, . \\ 
\end{array}
\right. 
$$

Supposons que le plan d'expérience soit équilibré, c'est à dire $p(y = 0) = p(y = 1) = 1/2$.  Dans ce cas, nous avons

$$
p({\bf x}, y | \theta) = p({\bf x} | y, \theta) p(y) = \left\{ 
\begin{array}{l}
q({\bf x}, \theta) / 2 & {\rm si } ~ y = 1 \, , \\ 
(1 - q({\bf x}, \theta)) / 2 & {\rm si } ~ y = 0 \, . \\ 
\end{array}
\right. 
$$
De manière plus synthétique, nous pouvons alors écrire

$$
p({\bf x}, y | \theta) = \frac12 q({\bf x}, \theta)^ y  (1 - q({\bf x}, \theta))^{1-y}
$$
Par indépendance, la loi de l'échantillon $({\bf x}_i, y_i)$ sera égale à 

$$
p(({\bf x}_i, y_i)_{i=1\dots,n} | \theta) = (\frac12)^n \prod_{i = 1}^n q({\bf x}_i, \theta)^{y_i}  (1 - q({\bf x}_i, \theta))^{1-y_i}
$$

Par définition la fonction de vraisemblance ${\cal L}(\theta)$ est égale à 
 
$$
\theta \mapsto {\cal L}(\theta) = p(({\bf x}_i, y_i)_{i=1\dots,n} | \theta) \, .
$$

Maximimiser cette fonction revient à chercher le paramètre $\hat \theta$ rendant les données les plus vraisemblables. De manière équivalente, on peut chercher à maximiser son logarithme, appelé log-vraisemblance, 

$$
 \hat \theta = \arg \max_{\theta} {\cal L}(\theta) = - \sum_{i=1}^n L(q({\bf x}_i, \theta), y_i) - n \log(2) \, .
$$
Dans cette formule $L(q({\bf x}), y)$ est la fonction de perte logarithmique. Ainsi nous voyons que maximiser la log-vraisemblance (divisée par $n$) est équivalent à minimiser la fonction de perte logarithmique 

$$
 \hat \theta = \arg \max_{\theta} {\cal L}(\theta) = \arg \min_{\theta} L(\theta) .
$$
Ce principe permet d'interpréter de manière probabiliste les paramètres recherchés en minimimisant la fonction de perte logarithmique. Il s'agit des paramètres pour lesquels la vraisemblance (la loi) des observations atteint une valeur maximale. 
