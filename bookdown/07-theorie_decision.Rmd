...
Pour simplifier les notations, nous nous plaçons dans le cas d'une classification binaire. Lorsque la variable $y$ est binaire (0 ou 1), la fonction s'écrit plus simplement 

$$L(q({\bf x}), y) = ( y - q({\bf x}) )^2 ,$$
où $q({\bf x})$ s'identifie à la probabilité de la classe 1. Pour minimiser la perte quadratique moyenne, il suffit de trouver la machine probabiliste $q({\bf x})$ minimisant l'espérance conditionnelle suivante
...
```{r}
  ...
  # condition initiale theta1 = -1, theta2 = 2
    objet <- optim(par = c(-1,2), fn = L, method = "BFGS")
