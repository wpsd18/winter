...
Minimiser l'erreur quadratique moyenne conduit au même résultat que pour la perte log-loss. Ce résultat justifie l'utilisation fréquence de l'erreur quadratique moyenne par les algorithmes d'apprentissage. En pratique, cette erreur est calculée à partir des données empiriques de la manière suivante
...
Pour généraliser ce résultat à $K$ classes, nous pouvons considérer un codage binaire de chaque classe. Ce type de codage est aussi appelé *factoriel* ou *categoriel* (en statistique), *dummy variable* ou *one-hot encoding* en apprentissage automatique. Toute classe est codée par un vecteur indicateur $e_k = (0,\dots, 1, \dots, 0)$, de dimension $K$ contenant des 0 et un (seul) 1 à la position $k$. Ainsi, $y = e_k$ indique que l'on se trouve dans la classe $k$.

A l'aide de ce codage des classes, nous pouvons définir la fonction de perte correspondant à l'erreur quadratique de la manière suivante 


$$L(q({\bf x}), y) = \sum_{k=1}^K ( y_k - q_k({\bf x}) )^2 ,$$
où $y \in \{e_1, \dots, e_K\}$ represente l'une des $K$ classes possibles.  

En suivant des arguments similaires à ceux utilisés pour deux classes,  la machine probabiliste optimale pour la fonction de perte moyenne correspond toujours à calculer la probabilité conditionnelle de chaque classe,
...
L'apprentissage automatique concerne donc essentiellement des machines (ou des prédicteurs) probabilistes. Il consiste en la minimisation d'un critère de perte à partir d'observations répétées du couple (variables, classe)  $({\bf x}_i, y_i)_{i = 1, \dots, n}$, supposées indépendantes les unes des autres.   
