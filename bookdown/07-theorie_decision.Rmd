...
* Formule de Bayes 
* Erreur de classification moyenne (_accuracy_) et perte d'information (_logloss_)
* Classifieur optimal 
* Surapprentissage et ensemble de test 

Ce chapitre et le suivant (apprentissage supervisé) forment le coeur de ce cours. Trois séances d'amphi y seront consacrées. 
...
Le script de simulation de cet histogramme peut s'écrire de la manière suivante
...
La fonction de perte appelée _logloss_  est fréquemment considérée par les algorithmes d'apprentissage probabiliste. Elle est définie de la manière suivante  
...
$$
où $q({\bf x})$ s'identifie à la probabilité de la classe 1, $q({\bf x}) = q_1({\bf x})$.

Nous allons montrer que minimiser l'erreur log-loss moyenne (_entropie_) revient à minimiser l'information perdue lorsque l'on approche la loi cible $p(y|{\bf x})$ par une machine probabiliste $q({\bf x})$. L'erreur moyenne associée à la fonction de perte log-loss est égale à
...
La grandeur qui apparaît dans le terme de droite de cette expression correspond à l'_entropie croisée_ des lois $p(y = k|{\bf x})$ et $q_k({\bf x})$, définies sur l'ensemble des K classes, $\{1,\dots,K\}$, de la manière suivante
...
**Résultat:** L'erreur logloss moyenne est minimale si et seulement, 

$$
q_k({\bf x}) = p(y = k|{\bf x}) \, , \quad k = 1, \dots, K.
$$

Minimiser l'erreur moyenne log-loss revient à minimiser l'information perdue lorsque l'on approche la loi cible $p(y|{\bf x})$ par une machine probabiliste $q({\bf x})$. Pour ce critère, nous obtenons la machine probabiliste optimale calcule la probabilité conditionnelle de chaque catégorie.

En pratique, l'erreur log-loss moyenne est calculée à partir des données d'apprentissage de la manière suivante 
...
$$
Le cas binaire sera le cas le plus fréquemment rencontré dans les exemples de ce cours.
...
$$
L(q({\bf x}), y) = ( y - q_y({\bf x}) )^2 \, .
$$
Pour une raison évidente, cette erreur est appelée perte quadratique ou perte $L^2$. 

Pour simplifier les notations, nous nous plaçons dans le cas d'une classification binaire. Lorsque la variable $y$ est binaire (0 ou 1), la fonction s'écrit tout simplement $L(q({\bf x}), y) = ( y - q({\bf x}) )^2$, où $q({\bf x})$ s'identifie à la probabilité de la classe 1. Pour minimiser la perte quadratique moyenne, il suffit de trouver la machine probabiliste $q({\bf x})$ minimisant l'espérance conditionnelle suivante

$$
\mathbb{E}[L(q({\bf x}),y) | {\bf x} ] = \mathbb{E}[(y - q({\bf x}))^2 | {\bf x} ]  \, .
$$

Faisons apparaître la probabilité conditionnelle de la classe 1, notée $p(y = 1 | {\bf x})$. Nous avons 
...
**Résultat:** Cela démontre que, pour l'erreur quadratique moyenne, la machine probabiliste optimale correspond à nouveau à calculer la probabilité conditionnelle de chaque classe,
...
consiste en la minimisation d'un critère de perte à partir d'observations répétées du couple classe/variables $(y_i,{\bf x}_i)_{i = 1, \dots, n}$, supposées indépendantes les uns des autres.   
...
Recherche la machine optimale par un algorithme d'apprentissage consiste alors à rechercher le paramètre $\theta$ minimisant la fonction $L(\theta)$ suivante

$$
L(\theta) = \frac1n \sum_{i = 1}^n L(q({\bf x}_i, \theta), y_i) \,.
$$
Le minimum optenu sera noté $\hat \theta$ 
...
Si la fonction de perte et le modèle de prédiction sont (plusieurs fois) différentiables, il est possible d'utiliser des algorithmes d'optimisation tels que l'_algorithme du gradient_ ou des _méthodes de Newton_ pour trouver des minima de la fonction de perte. Dans ce cours, nous utiliserons des algorithmes d'optimisation implantés dans des bibliothèques numériques. L'étude de ces méthodes n'est pas le sujet du cours, mais il faut retenir qu'il est très important de bien les choisir.  
...
Il est possible de minimiser cette fonction en utilisant la commande _optim()_ de R. Cette fonction constitue un exemple de programmation fonctionnelle. Pour cela nous utilisons un algorithme d'optimsation numérique classique, appelé méthode "BFGS",
...
Nous revenons aux modèles paramétriques qui constituent le coeur de ce cours, et nous abordons le problème de l'_ensemble test_. Jusqu'à maintenant nous avons considéré que les données pouvaient être utilisées pour l'apprentissage des machines. Dans cette section, nous allons voir qu'une partie des données disponibles doit être réservée pour évaluer la fonction de perte indépendamment de l'ensemble ayant servi pour ajuster le modèle (apprentissage).
...
Pour ajuster ce modèle, nul besoin d'algorithme numérique. La solution évidente consiste à poser $\hat\theta_{1,i} = y_i$ et $\hat\theta_{2,i} = x_i$, pour tout $i$ de 1 à $n$. Les erreurs de classification, de log-loss et de moindres carrés sont alors toutes égales à zéro, la valeur minimale absolue, pour l'ensemble d'apprentissage constitués des $n$ observations.
...
On considère un vecteur de mesures, ${\bf x}$, de dimension $D = 2$. On suppose que les données se répartissent en deux classes, $y = 0$ et $y = 1$. On suppose de plus que les données ont été générées en proportions égales dans chacune des deux classes, $p(y = 0) = p(y = 1)$.
