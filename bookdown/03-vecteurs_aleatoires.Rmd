...
- ${\bf X}$ : matrice de données de dimension $n\times D$ où $n$ représente la _taille d'un échantillon_ et $D$ la dimension des vecteurs aléatoires mesurés dans l'échantillon. L'étude de matrices de données ${\bf X}$ est l'un des objectifs de ce cours.
...
Les conventions précédentes sont universellement adoptées dans le domaine de l'apprentissage automatique.
...
$$
Losque les bornes ne sont pas indiquées, l'intégrale porte sur toutes les valeurs réelles. 
...
Le calcul théorique d'une espérance est rassurant, mais en général, calculer une telle intégrale de manière explicite est difficile. Nous devons garder à l'esprit que la notion signifie "valeur moyenne" la fonction $-\log p(x)$, lorsque les tirages sont de loi de densité $p(x)$. Dans les algorithmes, cette grandeur sera souvent évaluée empiriquement 
...
et en utilisons la fonction `dnorm()` pour calculer la densité de la loi normale. L'entropie est estimée de la manière suivante
...
$$
p({\bf x}) = \frac{1}{(2\pi \det{\bf C} )^{D/2}} 
