...
$$
Ainsi, les déplacements $d\theta$ conduisant aux plus grandes variations de la fonction $L(\theta)$ au voisinage de $\theta$ s'effectuent dans la direction parallèle au vecteur $\nabla L(\theta)$.  

La méthode de gradient est une méthode rudimentaire cherchant à suivre la ligne de plus grande pente afin de minimiser la fonction objectif $L(\theta)$. Il s'agit d'une méthode itérative qui met à jour le paramètre $\theta$ de la manière suivante 

$$
\theta^{t+1} = \theta^t - \eta \nabla L(\theta^t) \, ,
$$

où la constante $\eta$ est appelé _taux d'apprentissage_ (learning rate). Cette méthode peut être raffinée en utilisant les dérivées secondes de la fonction $L(\theta)$, lorsque le paramètre $\theta$ est de dimension raisonnable de l'ordre de quelques dizaines de coefficients. Mais en grande dimension, la méthode du du gradient reste favorisée dans les implémentations logicielles de réseaux neuronaux.
...
$$
ainsi que l'erreur quadratique associée
...
Dans ce cas, nous avons $\theta = (w_0,w_1)$ et il n'y a que deux dérivées à calculer pour obtenir le gradient de $L(w_0,w_1)$. Nous pouvons dériver la fonction $\ell(w_0,w_1)$ puis prendre l'espérance de chaque dérivée.
