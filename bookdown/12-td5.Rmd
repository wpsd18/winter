...
$$
\mathbb{E}[L(\phi({\bf x}), y)| {\bf x} ] \geq  q({\bf x}) = p(y = 1| {\bf x})
$$ 

pour toute fonction $\phi({\bf x}) \in [0,1]$. 
...
Les matrices ${\bf W_0^T}$ et ${\bf W_1^T}$ sont de dimensions respectives $D \times k$ et $k \times 1$, où $k$ est le nombre de neurones de la couche cachée. Les vecteurs ${\bf b_0}$ et ${\bf b_1}$ sont de dimensions respectives $k$ et $1$. La fonction _sigmoide_ est appliquée à chaque coordonnée des vecteurs.
...
$$
Le paramètre global est donc de dimension $(D + 2)k + 1$. Ainsi, en deux dimensions et en utilisant 20 neurones nous aurons 81 paramètres. 
...
L'optimisation est réalisée par un algorithme numérique (algorithme gradient stochastique par exemple) agissant directement sur le paramètre $\theta$. Le paramètre $\lambda$ est un paramètre régularisateur, dont le rôle est de contrebalancer une complexité trop élevée du modèle ($k$ trop grand). En d'autres termes, une grande valeur de $\lambda$ forcera certains poids à être très faibles. 

En considérant une grande valeur de $\lambda$, discuter l'effet de ce paramètre sur la fonction de perte quadratique. Comment chacun des deux termes de l'erreur quadratique moyenne est il sensible à ce paramètre ?

### Question 3

Commenter les codes et les résultats des expériences  suivantes où l'on fait varier les valeurs de $k$ et de $\lambda$
...
# Création d'une grille de valeurs tests pour les prédictions d'un modèle probabiliste 
####################################################################
...
Commenter le code suivant permettant de tester les prédictions de plusieurs modèles. Nous privilégierons un choix de $\lambda$ entre $0.8$ et $10^{-3}$ pour un nombre de neurones important (40).
