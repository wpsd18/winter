...
On dispose d'un échantillon de test. Quelle formule empirique permet de calculer l'erreur de prédiction (définie comme la valeur moyenne de la fonction de perte) ? On donnera une réponse pour $\phi({\bf x})$.
...
Etant donné l'observation ${\bf x}$, on considère l'erreur quadratique moyenne entre un prédicteur calculé à l'aide d'un ensemble d'apprentissage, noté $\hat{q}({\bf x})$, et le prédicteur optimal $q({\bf x})$

$$
{\rm EQM} = \mathbb{E}[ (\hat{q}({\bf x})  - q({\bf x}))^2 \, | {\bf x}]
$$

### Question 1 

Montrer que l'erreur quadratique moyenne se décompose de la manière suivante 

$$
{\rm EQM} = {\rm var}(\hat{q}({\bf x}) \, | {\bf x}) + (\mathbb{E}[\hat{q}({\bf x}) - q({\bf x}) \, | {\bf x}])^2 \, .
$$

Dans cette expression, l'incertitude ne porte que sur $\hat{q}({\bf x})$, variable aléatoire construite à partir d'un ensemble de données $({\bf x}_i, y_i)$ indépendantes de ${\bf x}$. Cette décomposition est souvent appelée **décomposition biais-variance**.
...
Les matrices ${\bf W_1^T}$ et ${\bf W_0^T}$ sont de dimensions respectives $D \times H$ et $H \times 1$, où $H$ est le nombre de neurones de la couche cachée. Les vecteurs ${\bf b_0}$ et ${\bf b_1}$ sont de dimensions respectives $H$ et $1$. La fonction _sigmoid_ est appliquée à chaque coordonnée des vecteurs.
...
# Simulation selon de la modèle HT
# 400 données sont équiréparties en 2 classes
...
Lorsque l'on utilise le paramètre de régularisation optimal, un affichage sous forme de carte de probabilité montre les détails des prédictions.
