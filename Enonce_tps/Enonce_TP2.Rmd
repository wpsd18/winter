...
* Calculer le nombre moyen de prédictions correctes effectuées sur l'ensemble d'apprentissage en considérant les probabilités de survies conditionnelles aux variables `gender` et `child`. Pour cela, on prédira *survie* lorsque la probabilité conditionnelle de survie est supérieure à 0.5.
...
où $y_i$ et  $p_i$ représentent l'indicateur de survie et la probabilité de survie de l'individu $i$. Quelle valeur de $L$ correspond à une prédiction au hasard (pile ou face) ? Quelle valeur de $L$ correspond à une prédiction effectuée à partir de la probabilité globale de survie ? 


* Calculer la perte **logloss** associée aux prédictions précédentes (pour éviter les erreurs numériques, on pourra remplacer les valeurs de probabilités nulles par $10^{-10}$). 
...
* Prédire la survie de chaque passager à partir des fréquences de survies conditionnelles aux variables `gender`, `child`, `class3` (repartir du code précédent).
...
* Calculer le nombre moyen de prédictions correctes effectuées et la perte logloss en considérant les fréquences de survies conditionnelles aux variables `gender`, `child`, `class3` (prédicteur fréquentiel).
...
* À l'aide du vecteur `pred.nnet`, calculer le nombre moyen de prédictions correctes effectuées par le modèle neuronal que vous venez d'ajuster et la perte logloss associée à ce modèle.
...
* Pour l'ensemble de test, evaluer le prédicteur fréquentiel s'appuyant sur les caractéristiques `gender`, `child`, `class3`. La fonction `eval_titanic` a accès aux données de survie pour les passagers de l'ensemble de test et renvoie les valeurs moyennes du taux de classification et de la perte logloss. Compléter les commentaires.
...
# commenter
 #mod <- nnet::nnet(x,  y, size = 3)
 pred.nnet <- predict(mod, tit_test)

# commenter
 isd::eval_titanic(pred.nnet)
```


* Comparer les résultats précédents aux valeurs obtenues pour l'ensemble d'apprentissage.  

* Quelles prédictions vous semblent les meilleures ? Celles obtenues par les probabilités empiriques ou celles obtenues par le modèle neuronal ? Quelles sont les différences de prédiction ? 



**Remarque :** Il est possible d'utiliser l'opérateur "pipe" pour la prediction
...
Compléter le document de rendu et le soumettre sur le site de dépot TEIDE au format *html*.
